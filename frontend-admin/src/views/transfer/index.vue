  ...
  <div class="transfer-page">
    ...
    <div class="table-card">
      ...
      <el-table :data="tableData" stripe v-loading="loading" empty-text="暂无数据">
        ...
        <el-table-column label="驳回理由" min-width="160" show-overflow-tooltip>
          <template #default="{ row }">
            <span v-if="row.status === 'REJECTED'" class="reject-reason-cell">
              {{ row.rejectReason || '-' }}
            </span>
    ...
    <el-dialog v-model="approveVisible" title="审批确认 - 库存核对" width="640px" destroy-on-close>
      ...
      <template #footer>
        ...
        <el-button type="danger" @click="switchToReject">填写驳回理由</el-button>
    ...
    <el-dialog v-model="rejectDialogVisible" title="驳回调拨单" width="520px" destroy-on-close>
      <el-form :model="rejectForm" :rules="rejectRules" ref="rejectFormRef" label-width="90px">
        <el-form-item label="调拨编号">
          <span class="plain-text">{{ rejectForm.transferNo }}</span>
        </el-form-item>
        <el-form-item label="驳回理由" prop="rejectReason">
          <el-input
            ...
            maxlength="500"
            show-word-limit
            placeholder="请详细说明驳回原因，以便发起人了解并修改"
          />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="detailVisible" title="调拨单详情" width="700px" destroy-on-close>
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item v-if="detail.status === 'REJECTED'" label="驳回理由" :span="2">
          <span class="reject-reason-display">{{ detail.rejectReason || '（未填写理由）' }}</span>
        </el-descriptions-item>
      </el-descriptions>
...
function openRejectDialog(row) {
  ...
  rejectForm.transferNo = row.transferNo
...
function switchToReject() {
  approveVisible.value = false
  if (approveTransferRow.value) {
    openRejectDialog(approveTransferRow.value)
  }
}
...
async function doReject() {
  ...
  try {
    await rejectTransfer(rejectForm.id, rejectForm.rejectReason)
